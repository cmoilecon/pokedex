export const games = [
  {
    id: "national",
    name: "Pokédex National",
    shortName: "National",
    subtitle: "Tous les Pokémon",
    dexUrl: "https://pokemondb.net/pokedex/national",
    coverPokemonIds: [1, 4, 7]
  },
  {
    id: "lets-go-pikachu-eevee",
    name: "Pokémon Let's Go Pikachu/Évoli",
    shortName: "Let's Go",
    subtitle: "Kanto Pokédex",
    dexUrl: "https://pokemondb.net/pokedex/game/lets-go-pikachu-eevee",
    coverPokemonIds: [1, 4, 7]
  },
  {
    id: "sword-shield",
    name: "Pokémon Épée/Bouclier",
    shortName: "Épée/Bouclier",
    subtitle: "Galar Pokédex",
    dexUrl: "https://pokemondb.net/pokedex/game/sword-shield",
    coverPokemonIds: [1, 4, 7]
  },
  {
    id: "isle-of-armor",
    name: "Épée/Bouclier — Isolarmure",
    shortName: "Isolarmure",
    subtitle: "Isle of Armor Pokédex",
    dexUrl: "https://pokemondb.net/pokedex/game/sword-shield/isle-of-armor",
    coverPokemonIds: [1, 2, 3]
  },
  {
    id: "crown-tundra",
    name: "Épée/Bouclier — Couronneige",
    shortName: "Couronneige",
    subtitle: "Crown Tundra Pokédex",
    dexUrl: "https://pokemondb.net/pokedex/game/sword-shield/crown-tundra",
    coverPokemonIds: [1, 2, 3]
  },
  {
    id: "brilliant-diamond-shining-pearl",
    name: "Pokémon Diamant Étincelant/Perle Scintillante",
    shortName: "DEPS",
    subtitle: "Sinnoh Pokédex",
    dexUrl: "https://pokemondb.net/pokedex/game/brilliant-diamond-shining-pearl",
    coverPokemonIds: [1, 4, 7]
  },
  {
    id: "legends-arceus",
    name: "Pokémon Legends: Arceus",
    shortName: "Legends: Arceus",
    subtitle: "Hisui Pokédex",
    dexUrl: "https://pokemondb.net/pokedex/game/legends-arceus",
    coverPokemonIds: [1, 4, 7]
  },
  {
    id: "scarlet-violet",
    name: "Pokémon Écarlate/Violet",
    shortName: "Écarlate/Violet",
    subtitle: "Paldea Pokédex",
    dexUrl: "https://pokemondb.net/pokedex/game/scarlet-violet",
    coverPokemonIds: [1, 4, 7]
  },
  {
    id: "teal-mask",
    name: "Écarlate/Violet — Le Masque Turquoise",
    shortName: "Septentria",
    subtitle: "Kitakami Pokédex",
    dexUrl: "https://pokemondb.net/pokedex/game/scarlet-violet/teal-mask",
    coverPokemonIds: [1, 2, 3]
  },
  {
    id: "indigo-disk",
    name: "Écarlate/Violet — Le Disque Indigo",
    shortName: "Institut Myrtille",
    subtitle: "Blueberry Pokédex",
    dexUrl: "https://pokemondb.net/pokedex/game/scarlet-violet/indigo-disk",
    coverPokemonIds: [1, 4, 7]
  },
  {
    id: "legends-z-a",
    name: "Pokémon Legends: Z-A",
    shortName: "Legends: Z-A",
    subtitle: "Kalos Pokédex",
    dexUrl: "https://pokemondb.net/pokedex/game/legends-z-a",
    coverPokemonIds: [1, 4, 7]
  },
  {
    id: "mega-dimension",
    name: "Legends: Z-A — Mega Dimension",
    shortName: "Mega Dimension",
    subtitle: "Hyperspace Pokédex",
    dexUrl: "https://pokemondb.net/pokedex/game/legends-z-a/mega-dimension",
    coverPokemonIds: [121, 125, 132]
  }
];